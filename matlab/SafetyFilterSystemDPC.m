classdef SafetyFilterSystemDPC < matlab.System
   properties
    ...
    methods (Access = protected)
        ...
        function [m, dv_pcc_f, dm_corr, active, lambda] = outputImpl(obj, ~)
            m = obj.m;
            dv_pcc_f = obj.dv_pcc_f;
            dm_corr = obj.dm_corr;
            active = obj.active;
            lambda = obj.lambda;
        end
        ...
        function updateImpl(obj, u)
            ...
            if false
            % if V <= 0
            % if (obj.active == 0 && B2 <= 0) || (obj.active == 1 && V <= 0)
            % if V2 <= 0
